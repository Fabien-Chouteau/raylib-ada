...
is
  ...
  type C_String_Array is
   array (Interfaces.C.unsigned) of aliased Interfaces.C.Strings.chars_ptr
  with Convention => C;
  ...
  FLAG_VSYNC_HINT               : constant ConfigFlags :=
   64; -- Set to try enabling V-Sync on GPU
  FLAG_FULLSCREEN_MODE          : constant ConfigFlags :=
   2; -- Set to run program in fullscreen
  FLAG_WINDOW_RESIZABLE         : constant ConfigFlags :=
   4; -- Set to allow resizable window
  FLAG_WINDOW_UNDECORATED       : constant ConfigFlags :=
   8; -- Set to disable window decoration (frame and buttons)
  FLAG_WINDOW_HIDDEN            : constant ConfigFlags :=
   128; -- Set to hide window
  FLAG_WINDOW_MINIMIZED         : constant ConfigFlags :=
   512; -- Set to minimize window (iconify)
  FLAG_WINDOW_MAXIMIZED         : constant ConfigFlags :=
   1024; -- Set to maximize window (expanded to monitor)
  FLAG_WINDOW_UNFOCUSED         : constant ConfigFlags :=
   2048; -- Set to window non focused
  FLAG_WINDOW_TOPMOST           : constant ConfigFlags :=
   4096; -- Set to window always on top
  FLAG_WINDOW_ALWAYS_RUN        : constant ConfigFlags :=
   256; -- Set to allow windows running while minimized
  FLAG_WINDOW_TRANSPARENT       : constant ConfigFlags :=
   16; -- Set to allow transparent framebuffer
  FLAG_WINDOW_HIGHDPI           : constant ConfigFlags :=
   8192; -- Set to support HighDPI
  FLAG_WINDOW_MOUSE_PASSTHROUGH : constant ConfigFlags :=
   16384; -- Set to support mouse passthrough, only supported when FLAG_WINDOW_UNDECORATED
  FLAG_BORDERLESS_WINDOWED_MODE : constant ConfigFlags :=
   32768; -- Set to run program in borderless windowed mode
  FLAG_MSAA_4X_HINT             : constant ConfigFlags :=
   32; -- Set to try enabling MSAA 4X
  FLAG_INTERLACED_HINT          : constant ConfigFlags :=
   65536; -- Set to try enabling interlaced video format (for V3D)
  ...
  KEY_NULL          : constant KeyboardKey :=
   0; -- Key: NULL, used for no key pressed
  ...
  KEY_VOLUME_UP     : constant KeyboardKey :=
   24; -- Key: Android volume up button
  KEY_VOLUME_DOWN   : constant KeyboardKey :=
   25; -- Key: Android volume down button
  ...
  MOUSE_BUTTON_MIDDLE  : constant MouseButton :=
   2; -- Mouse button middle (pressed wheel)
  MOUSE_BUTTON_SIDE    : constant MouseButton :=
   3; -- Mouse button side (advanced mouse device)
  MOUSE_BUTTON_EXTRA   : constant MouseButton :=
   4; -- Mouse button extra (advanced mouse device)
  MOUSE_BUTTON_FORWARD : constant MouseButton :=
   5; -- Mouse button forward (advanced mouse device)
  MOUSE_BUTTON_BACK    : constant MouseButton :=
   6; -- Mouse button back (advanced mouse device)
  ...
  GAMEPAD_BUTTON_UNKNOWN          : constant GamepadButton :=
   0; -- Unknown button, just for error checking
  GAMEPAD_BUTTON_LEFT_FACE_UP     : constant GamepadButton :=
   1; -- Gamepad left DPAD up button
  GAMEPAD_BUTTON_LEFT_FACE_RIGHT  : constant GamepadButton :=
   2; -- Gamepad left DPAD right button
  GAMEPAD_BUTTON_LEFT_FACE_DOWN   : constant GamepadButton :=
   3; -- Gamepad left DPAD down button
  GAMEPAD_BUTTON_LEFT_FACE_LEFT   : constant GamepadButton :=
   4; -- Gamepad left DPAD left button
  GAMEPAD_BUTTON_RIGHT_FACE_UP    : constant GamepadButton :=
   5; -- Gamepad right button up (i.e. PS3: Triangle, Xbox: Y)
  GAMEPAD_BUTTON_RIGHT_FACE_RIGHT : constant GamepadButton :=
   6; -- Gamepad right button right (i.e. PS3: Square, Xbox: X)
  GAMEPAD_BUTTON_RIGHT_FACE_DOWN  : constant GamepadButton :=
   7; -- Gamepad right button down (i.e. PS3: Cross, Xbox: A)
  GAMEPAD_BUTTON_RIGHT_FACE_LEFT  : constant GamepadButton :=
   8; -- Gamepad right button left (i.e. PS3: Circle, Xbox: B)
  GAMEPAD_BUTTON_LEFT_TRIGGER_1   : constant GamepadButton :=
   9; -- Gamepad top/back trigger left (first), it could be a trailing button
  GAMEPAD_BUTTON_LEFT_TRIGGER_2   : constant GamepadButton :=
   10; -- Gamepad top/back trigger left (second), it could be a trailing button
  GAMEPAD_BUTTON_RIGHT_TRIGGER_1  : constant GamepadButton :=
   11; -- Gamepad top/back trigger right (one), it could be a trailing button
  GAMEPAD_BUTTON_RIGHT_TRIGGER_2  : constant GamepadButton :=
   12; -- Gamepad top/back trigger right (second), it could be a trailing button
  GAMEPAD_BUTTON_MIDDLE_LEFT      : constant GamepadButton :=
   13; -- Gamepad center buttons, left one (i.e. PS3: Select)
  GAMEPAD_BUTTON_MIDDLE           : constant GamepadButton :=
   14; -- Gamepad center buttons, middle one (i.e. PS3: PS, Xbox: XBOX)
  GAMEPAD_BUTTON_MIDDLE_RIGHT     : constant GamepadButton :=
   15; -- Gamepad center buttons, right one (i.e. PS3: Start)
  GAMEPAD_BUTTON_LEFT_THUMB       : constant GamepadButton :=
   16; -- Gamepad joystick pressed button left
  GAMEPAD_BUTTON_RIGHT_THUMB      : constant GamepadButton :=
   17; -- Gamepad joystick pressed button right

  type GamepadAxis is new Interfaces.C.int;
  --  Gamepad axis

  GAMEPAD_AXIS_LEFT_X        : constant GamepadAxis :=
   0; -- Gamepad left stick X axis
  GAMEPAD_AXIS_LEFT_Y        : constant GamepadAxis :=
   1; -- Gamepad left stick Y axis
  GAMEPAD_AXIS_RIGHT_X       : constant GamepadAxis :=
   2; -- Gamepad right stick X axis
  GAMEPAD_AXIS_RIGHT_Y       : constant GamepadAxis :=
   3; -- Gamepad right stick Y axis
  GAMEPAD_AXIS_LEFT_TRIGGER  : constant GamepadAxis :=
   4; -- Gamepad back trigger left, pressure level: [1..-1]
  GAMEPAD_AXIS_RIGHT_TRIGGER : constant GamepadAxis :=
  ...
  for FontType use (FONT_DEFAULT => 0, FONT_BITMAP => 1, FONT_SDF => 2);
  ...
  for CameraProjection use (CAMERA_PERSPECTIVE => 0, CAMERA_ORTHOGRAPHIC => 1);
  ...
  type LoadFileDataCallback is
   access function
    (fileName : Interfaces.C.Strings.chars_ptr;
     dataSize : access Interfaces.C.int) return access Interfaces.C.char

  with Convention => C;
  --  FileIO: Load binary data

  type SaveFileDataCallback is
   access function
    (fileName : Interfaces.C.Strings.chars_ptr;
     data     : System.Address;
     dataSize : Interfaces.C.int) return Interfaces.C.C_bool

  with Convention => C;
  --  FileIO: Save binary data

  type LoadFileTextCallback is
   access function
    (fileName : Interfaces.C.Strings.chars_ptr)
  ...
  type SaveFileTextCallback is
   access function
    (fileName : Interfaces.C.Strings.chars_ptr;
     text     : Interfaces.C.Strings.chars_ptr) return Interfaces.C.C_bool

  with Convention => C;
  --  FileIO: Save text data

  type AudioCallback is
   access procedure
    (bufferData : System.Address; frames : Interfaces.C.unsigned)
  ...
  type Camera3D is record
    ...
    fovy       :
     Interfaces
      .C
      .C_float; -- Camera field-of-view aperture in Y (degrees) in perspective, used as near plane width in orthographic
    projection :
     CameraProjection; -- Camera projection: CAMERA_PERSPECTIVE or CAMERA_ORTHOGRAPHIC
  end record
  with Convention => C_Pass_By_Copy;
  ...
  type Camera2D is record
    ...
    zoom     :
     Interfaces.C.C_float; -- Camera zoom (scaling), should be 1.0f by default
  end record
  with Convention => C_Pass_By_Copy;
  ...
  type Mesh is record
    ...
    triangleCount :
     Interfaces.C.int; -- Number of triangles stored (indexed or not)
    vertices      :
     access
      Interfaces
       .C
       .C_float; -- Vertex position (XYZ - 3 components per vertex) (shader-location = 0)
    texcoords     :
     access
      Interfaces
       .C
       .C_float; -- Vertex texture coordinates (UV - 2 components per vertex) (shader-location = 1)
    texcoords2    :
     access
      Interfaces
       .C
       .C_float; -- Vertex texture second coordinates (UV - 2 components per vertex) (shader-location = 5)
    normals       :
     access
      Interfaces
       .C
       .C_float; -- Vertex normals (XYZ - 3 components per vertex) (shader-location = 2)
    tangents      :
     access
      Interfaces
       .C
       .C_float; -- Vertex tangents (XYZW - 4 components per vertex) (shader-location = 4)
    colors        :
     access
      Interfaces
       .C
       .char; -- Vertex colors (RGBA - 4 components per vertex) (shader-location = 3)
    indices       :
     access
      Interfaces.C.short; -- Vertex indices (in case vertex data comes indexed)
    animVertices  :
     access
      Interfaces
       .C
       .C_float; -- Animated vertex positions (after bones transformations)
    animNormals   :
     access
      Interfaces.C.C_float; -- Animated normals (after bones transformations)
    boneIds       :
     access
      Interfaces
       .C
       .char; -- Vertex bone ids, max 255 bone ids, up to 4 bones influence by vertex (skinning)
    boneWeights   :
     access
      Interfaces
       .C
       .C_float; -- Vertex bone weight, up to 4 bones influence by vertex (skinning)
    vaoId         : Interfaces.C.unsigned; -- OpenGL Vertex Array Object id
    vboId         :
     access
      Interfaces
       .C
       .unsigned; -- OpenGL Vertex Buffer Objects id (default vertex data)
  end record
  with Convention => C_Pass_By_Copy;
  ...
  type Shader is record
    ...
    locs :
     access
      ShaderLocationArray; -- Shader locations array (RL_MAX_SHADER_LOCATIONS)
  end record
  with Convention => C_Pass_By_Copy;
  ...
  type Material is record
    ...
    maps     :
     access MaterialMapArray; -- Material maps array (MAX_MATERIAL_MAPS)
  ...
  type Wave is record
    frameCount :
     Interfaces.C.unsigned; -- Total number of frames (considering channels)
    sampleRate : Interfaces.C.unsigned; -- Frequency (samples per second)
    sampleSize :
     Interfaces
      .C
      .unsigned; -- Bit depth (bits per sample): 8, 16, 32 (24 not supported)
    channels   :
     Interfaces.C.unsigned; -- Number of channels (1-mono, 2-stereo, ...)
  ...
  type AudioStream is record
    buffer     :
     System.Address; -- Pointer to internal data used by the audio system
    processor  :
     System
      .Address; -- Pointer to internal data processor, useful for audio effects
    sampleRate : Interfaces.C.unsigned; -- Frequency (samples per second)
    sampleSize :
     Interfaces
      .C
  ...
  type VrDeviceInfo is record
    hResolution            :
     Interfaces.C.int; -- Horizontal resolution in pixels
    ...
    eyeToScreenDistance    :
     Interfaces.C.C_float; -- Distance between eye and display in meters
    lensSeparationDistance :
     Interfaces.C.C_float; -- Lens separation distance in meters
    interpupillaryDistance :
     Interfaces.C.C_float; -- IPD (distance between pupils) in meters
    lensDistortionValues   : Float4; -- Lens distortion constant parameters
    chromaAbCorrection     :
     Float4; -- Chromatic aberration correction parameters
  end record
  with Convention => C_Pass_By_Copy;
  ...
  with Convention => C_Pass_By_Copy;
  type AutomationEvent_Array is
   array (Interfaces.C.unsigned) of AutomationEvent
  with Convention => C;

  type AutomationEventList is record
    capacity :
     Interfaces.C.unsigned; -- Events max entries (MAX_AUTOMATION_EVENTS)
  ...
  procedure InitWindow
   (width : Interfaces.C.int; height : Interfaces.C.int; title : String);
  --  Initialize window and OpenGL context
  ...
  function IsWindowState
   (flag : Interfaces.C.unsigned) return Interfaces.C.C_bool;
  ...
  procedure SetWindowMinSize
   (width : Interfaces.C.int; height : Interfaces.C.int);
  --  Set window minimum dimensions (for FLAG_WINDOW_RESIZABLE)
  pragma Import (C, SetWindowMinSize, "SetWindowMinSize");

  procedure SetWindowMaxSize
   (width : Interfaces.C.int; height : Interfaces.C.int);
  ...
  function LoadShader
   (vsFileName : Interfaces.C.Strings.chars_ptr;
    fsFileName : Interfaces.C.Strings.chars_ptr) return Shader;
  ...
  function LoadShaderFromMemory
   (vsCode : Interfaces.C.Strings.chars_ptr;
    fsCode : Interfaces.C.Strings.chars_ptr) return Shader;
  --  Load shader from code strings and bind default locations
  pragma Import (C, LoadShaderFromMemory, "LoadShaderFromMemory");

  function LoadShaderFromMemory
   (vsCode : String; fsCode : String) return Shader;
  --  Load shader from code strings and bind default locations
  ...
  function GetShaderLocation
   (shader_p : Shader; uniformName : Interfaces.C.Strings.chars_ptr)
    return Interfaces.C.int;
  --  Get shader uniform location
  pragma Import (C, GetShaderLocation, "GetShaderLocation");

  function GetShaderLocation
   (shader_p : Shader; uniformName : String) return Interfaces.C.int;
  --  Get shader uniform location

  function GetShaderLocationAttrib
   (shader_p : Shader; attribName : Interfaces.C.Strings.chars_ptr)
    return Interfaces.C.int;
  --  Get shader attribute location
  pragma Import (C, GetShaderLocationAttrib, "GetShaderLocationAttrib");

  function GetShaderLocationAttrib
   (shader_p : Shader; attribName : String) return Interfaces.C.int;
  --  Get shader attribute location

  procedure SetShaderValue
   (shader_p    : Shader;
    locIndex    : Interfaces.C.int;
  ...
  procedure SetShaderValueMatrix
   (shader_p : Shader; locIndex : Interfaces.C.int; mat : Matrix);
  --  Set shader uniform value (matrix 4x4)
  pragma Import (C, SetShaderValueMatrix, "SetShaderValueMatrix");

  procedure SetShaderValueTexture
   (shader_p : Shader; locIndex : Interfaces.C.int; texture_p : Texture);
  ...
  function GetMouseRay
   (mousePosition : Vector2; camera_p : Camera3D) return Ray;
  ...
  function GetWorldToScreen
   (position : Vector3; camera_p : Camera3D) return Vector2;
  --  Get the screen space position for a 3d world space position
  pragma Import (C, GetWorldToScreen, "GetWorldToScreen");

  function GetScreenToWorld2D
   (position : Vector2; camera_p : Camera2D) return Vector2;
  ...
  function GetWorldToScreenEx
   (position : Vector3;
    camera_p : Camera3D;
    width    : Interfaces.C.int;
    height   : Interfaces.C.int) return Vector2;
  ...
  function GetRandomValue
   (min : Interfaces.C.int; max : Interfaces.C.int) return Interfaces.C.int;
  ...
  function LoadRandomSequence
   (count : Interfaces.C.unsigned;
    ...
    max   : Interfaces.C.int) return access Interfaces.C.int;
  ...
  function MemRealloc
   (ptr : System.Address; size : Interfaces.C.unsigned) return System.Address;
  ...
  function LoadFileData
   (fileName : String; dataSize : access Interfaces.C.int)
    return access Interfaces.C.char;
  --  Load file data as byte array (read)
  ...
  function SaveFileData
   (fileName : String; data : System.Address; dataSize : Interfaces.C.int)
    return Interfaces.C.C_bool;
  --  Save data to file from byte array (write), returns true on success

  function ExportDataAsCode
   (data     : System.Address;
    dataSize : Interfaces.C.int;
    fileName : Interfaces.C.Strings.chars_ptr) return Interfaces.C.C_bool;
  --  Export data to code (.h), returns true on success
  pragma Import (C, ExportDataAsCode, "ExportDataAsCode");

  function ExportDataAsCode
   (data : System.Address; dataSize : Interfaces.C.int; fileName : String)
    return Interfaces.C.C_bool;
  --  Export data to code (.h), returns true on success
  ...
  function SaveFileText
   (fileName : String; text : String) return Interfaces.C.C_bool;
  --  Save text data to file (write), string must be '\0' terminated, returns true on success
  ...
  function DirectoryExists
   (dirPath : Interfaces.C.Strings.chars_ptr) return Interfaces.C.C_bool;
  ...
  function IsFileExtension
   (fileName : Interfaces.C.Strings.chars_ptr;
    ext      : Interfaces.C.Strings.chars_ptr) return Interfaces.C.C_bool;
  --  Check file extension (including point: .png, .wav)
  pragma Import (C, IsFileExtension, "IsFileExtension");

  function IsFileExtension
   (fileName : String; ext : String) return Interfaces.C.C_bool;
  --  Check file extension (including point: .png, .wav)

  function GetFileLength
   (fileName : Interfaces.C.Strings.chars_ptr) return Interfaces.C.int;
  ...
  function ChangeDirectory
   (dir : Interfaces.C.Strings.chars_ptr) return Interfaces.C.C_bool;
  ...
  function IsPathFile
   (path : Interfaces.C.Strings.chars_ptr) return Interfaces.C.C_bool;
  ...
  function LoadDirectoryFiles
   (dirPath : Interfaces.C.Strings.chars_ptr) return FilePathList;
  ...
  function LoadDirectoryFilesEx
   (basePath    : Interfaces.C.Strings.chars_ptr;
    filter      : Interfaces.C.Strings.chars_ptr;
    scanSubdirs : Interfaces.C.C_bool) return FilePathList;
  --  Load directory filepaths with extension filtering and recursive directory scan
  pragma Import (C, LoadDirectoryFilesEx, "LoadDirectoryFilesEx");

  function LoadDirectoryFilesEx
   (basePath : String; filter : String; scanSubdirs : Interfaces.C.C_bool)
    return FilePathList;
  --  Load directory filepaths with extension filtering and recursive directory scan
  ...
  function GetFileModTime
   (fileName : Interfaces.C.Strings.chars_ptr) return Interfaces.C.long;
  ...
  function CompressData
   (data         : System.Address;
    dataSize     : Interfaces.C.int;
    compDataSize : access Interfaces.C.int) return access Interfaces.C.char;
  --  Compress data (DEFLATE algorithm), memory must be MemFree()
  pragma Import (C, CompressData, "CompressData");

  function DecompressData
   (compData     : System.Address;
    compDataSize : Interfaces.C.int;
    dataSize     : access Interfaces.C.int) return access Interfaces.C.char;
  ...
  function EncodeDataBase64
   (data       : System.Address;
    dataSize   : Interfaces.C.int;
    outputSize : access Interfaces.C.int)
    return Interfaces.C.Strings.chars_ptr;
  --  Encode data to Base64 string, memory must be MemFree()
  pragma Import (C, EncodeDataBase64, "EncodeDataBase64");

  function DecodeDataBase64
   (data : System.Address; outputSize : access Interfaces.C.int)
    return access Interfaces.C.char;
  --  Decode Base64 string data, memory must be MemFree()
  pragma Import (C, DecodeDataBase64, "DecodeDataBase64");

  function LoadAutomationEventList
   (fileName : Interfaces.C.Strings.chars_ptr) return AutomationEventList;
  --  Load automation events list from file, NULL for empty list, capacity = MAX_AUTOMATION_EVENTS
  pragma Import (C, LoadAutomationEventList, "LoadAutomationEventList");

  function LoadAutomationEventList
   (fileName : String) return AutomationEventList;
  --  Load automation events list from file, NULL for empty list, capacity = MAX_AUTOMATION_EVENTS
  ...
  function ExportAutomationEventList
   (list : AutomationEventList; fileName : Interfaces.C.Strings.chars_ptr)
    return Interfaces.C.C_bool;
  --  Export automation events list as text file
  pragma Import (C, ExportAutomationEventList, "ExportAutomationEventList");

  function ExportAutomationEventList
   (list : AutomationEventList; fileName : String) return Interfaces.C.C_bool;
  --  Export automation events list as text file
  ...
  pragma
   Import (C, SetAutomationEventBaseFrame, "SetAutomationEventBaseFrame");

  procedure StartAutomationEventRecording;
  --  Start recording automation events (AutomationEventList must be set)
  pragma
   Import (C, StartAutomationEventRecording, "StartAutomationEventRecording");

  procedure StopAutomationEventRecording;
  --  Stop recording automation events
  pragma
   Import (C, StopAutomationEventRecording, "StopAutomationEventRecording");
  ...
  function IsGamepadAvailable
   (gamepad : Interfaces.C.int) return Interfaces.C.C_bool;
  ...
  function IsGamepadButtonPressed
   (gamepad : Interfaces.C.int; button : GamepadButton)
    return Interfaces.C.C_bool;
  ...
  function GetGamepadAxisMovement
   (gamepad : Interfaces.C.int; axis : GamepadAxis)
    return Interfaces.C.C_float;
  --  Get axis movement value for a gamepad axis
  pragma Import (C, GetGamepadAxisMovement, "GetGamepadAxisMovement");

  function SetGamepadMappings
   (mappings : Interfaces.C.Strings.chars_ptr) return Interfaces.C.int;
  ...
  function IsMouseButtonPressed
   (button : MouseButton) return Interfaces.C.C_bool;
  ...
  procedure SetMouseOffset
   (offsetX : Interfaces.C.int; offsetY : Interfaces.C.int);
  --  Set mouse offset
  pragma Import (C, SetMouseOffset, "SetMouseOffset");

  procedure SetMouseScale
   (scaleX : Interfaces.C.C_float; scaleY : Interfaces.C.C_float);
  --  Set mouse scaling
  ...
  function IsGestureDetected
   (gesture : Interfaces.C.unsigned) return Interfaces.C.C_bool;
  ...
  procedure UpdateCameraPro
   (camera_p : access Camera3D;
    movement : Vector3;
    rotation : Vector3;
  ...
  procedure DrawPixel
   (posX : Interfaces.C.int; posY : Interfaces.C.int; color_p : Color);
  --  Draw a pixel
  ...
  procedure DrawLine
   (startPosX : Interfaces.C.int;
    startPosY : Interfaces.C.int;
    endPosX   : Interfaces.C.int;
    endPosY   : Interfaces.C.int;
    color_p   : Color);
  --  Draw a line
  ...
  procedure DrawLineStrip
   (points : access Vector2; pointCount : Interfaces.C.int; color_p : Color);
  ...
  procedure DrawCircle
   (centerX : Interfaces.C.int;
    centerY : Interfaces.C.int;
  ...
  procedure DrawCircleSector
   (center     : Vector2;
    radius     : Interfaces.C.C_float;
    startAngle : Interfaces.C.C_float;
    endAngle   : Interfaces.C.C_float;
  ...
  procedure DrawCircleV
   (center : Vector2; radius : Interfaces.C.C_float; color_p : Color);
  ...
  procedure DrawEllipse
   (centerX : Interfaces.C.int;
    centerY : Interfaces.C.int;
    radiusH : Interfaces.C.C_float;
    radiusV : Interfaces.C.C_float;
    color_p : Color);
  --  Draw ellipse
  ...
  procedure DrawRing
   (center      : Vector2;
    innerRadius : Interfaces.C.C_float;
    outerRadius : Interfaces.C.C_float;
    startAngle  : Interfaces.C.C_float;
    endAngle    : Interfaces.C.C_float;
    segments    : Interfaces.C.int;
    color_p     : Color);
  ...
  procedure DrawRectangle
   (posX    : Interfaces.C.int;
    posY    : Interfaces.C.int;
    width   : Interfaces.C.int;
    height  : Interfaces.C.int;
    color_p : Color);
  ...
  procedure DrawRectangleV
   (position : Vector2; size : Vector2; color_p : Color);
  ...
  procedure DrawRectangleLinesEx
   (rec : Rectangle; lineThick : Interfaces.C.C_float; color_p : Color);
  ...
  procedure DrawSplineSegmentLinear
   (p1 : Vector2; p2 : Vector2; thick : Interfaces.C.C_float; color_p : Color);
  ...
  pragma
   Import (C, DrawSplineSegmentCatmullRom, "DrawSplineSegmentCatmullRom");
  ...
  pragma
   Import
    (C, DrawSplineSegmentBezierQuadratic, "DrawSplineSegmentBezierQuadratic");
  ...
  pragma
   Import (C, DrawSplineSegmentBezierCubic, "DrawSplineSegmentBezierCubic");

  function GetSplinePointLinear
   (startPos : Vector2; endPos : Vector2; t : Interfaces.C.C_float)
    return Vector2;
  --  Get (evaluate) spline point: Linear
  pragma Import (C, GetSplinePointLinear, "GetSplinePointLinear");

  function GetSplinePointBasis
   (p1 : Vector2;
    p2 : Vector2;
    p3 : Vector2;
    p4 : Vector2;
    t  : Interfaces.C.C_float) return Vector2;
  --  Get (evaluate) spline point: B-Spline
  pragma Import (C, GetSplinePointBasis, "GetSplinePointBasis");

  function GetSplinePointCatmullRom
   (p1 : Vector2;
    p2 : Vector2;
    p3 : Vector2;
    p4 : Vector2;
  ...
  function GetSplinePointBezierQuad
   (p1 : Vector2; c2 : Vector2; p3 : Vector2; t : Interfaces.C.C_float)
    return Vector2;
  --  Get (evaluate) spline point: Quadratic Bezier
  pragma Import (C, GetSplinePointBezierQuad, "GetSplinePointBezierQuad");

  function GetSplinePointBezierCubic
   (p1 : Vector2;
    c2 : Vector2;
    c3 : Vector2;
    p4 : Vector2;
  ...
  function CheckCollisionRecs
   (rec1 : Rectangle; rec2 : Rectangle) return Interfaces.C.C_bool;
  ...
  pragma
   Import (C, CheckCollisionPointTriangle, "CheckCollisionPointTriangle");

  function CheckCollisionPointPoly
   (point : Vector2; points : access Vector2; pointCount : Interfaces.C.int)
    return Interfaces.C.C_bool;
  ...
  function LoadImageRaw
   (fileName   : Interfaces.C.Strings.chars_ptr;
    ...
    height     : Interfaces.C.int;
    format     : PixelFormat;
    headerSize : Interfaces.C.int) return Image;
  ...
  function LoadImageAnim
   (fileName : Interfaces.C.Strings.chars_ptr;
    frames   : access Interfaces.C.int) return Image;
  --  Load image sequence from file (frames appended to image.data)
  pragma Import (C, LoadImageAnim, "LoadImageAnim");

  function LoadImageAnim
   (fileName : String; frames : access Interfaces.C.int) return Image;
  --  Load image sequence from file (frames appended to image.data)

  function LoadImageFromMemory
   (fileType : Interfaces.C.Strings.chars_ptr;
    fileData : System.Address;
    dataSize : Interfaces.C.int) return Image;
  --  Load image from memory buffer, fileType refers to extension: i.e. '.png'
  pragma Import (C, LoadImageFromMemory, "LoadImageFromMemory");

  function LoadImageFromMemory
   (fileType : String; fileData : System.Address; dataSize : Interfaces.C.int)
    return Image;
  --  Load image from memory buffer, fileType refers to extension: i.e. '.png'
  ...
  function ExportImage
   (image_p : Image; fileName : Interfaces.C.Strings.chars_ptr)
    return Interfaces.C.C_bool;
  --  Export image data to file, returns true on success
  pragma Import (C, ExportImage, "ExportImage");

  function ExportImage
   (image_p : Image; fileName : String) return Interfaces.C.C_bool;
  --  Export image data to file, returns true on success

  function ExportImageToMemory
   (image_p  : Image;
    fileType : Interfaces.C.Strings.chars_ptr;
    fileSize : access Interfaces.C.int) return access Interfaces.C.char;
  --  Export image to memory buffer
  pragma Import (C, ExportImageToMemory, "ExportImageToMemory");

  function ExportImageToMemory
   (image_p : Image; fileType : String; fileSize : access Interfaces.C.int)
    return access Interfaces.C.char;
  --  Export image to memory buffer
  ...
  function GenImageColor
   (width : Interfaces.C.int; height : Interfaces.C.int; color_p : Color)
    return Image;
  --  Generate image: plain color
  pragma Import (C, GenImageColor, "GenImageColor");

  function GenImageGradientLinear
   (width     : Interfaces.C.int;
    height    : Interfaces.C.int;
    direction : Interfaces.C.int;
    start     : Color;
    end_p     : Color) return Image;
  ...
  function GenImageChecked
   (width   : Interfaces.C.int;
    height  : Interfaces.C.int;
    checksX : Interfaces.C.int;
    checksY : Interfaces.C.int;
    col1    : Color;
    col2    : Color) return Image;
  ...
  function GenImageWhiteNoise
   (width  : Interfaces.C.int;
    height : Interfaces.C.int;
    factor : Interfaces.C.C_float) return Image;
  ...
  function GenImagePerlinNoise
   (width   : Interfaces.C.int;
    height  : Interfaces.C.int;
    offsetX : Interfaces.C.int;
    offsetY : Interfaces.C.int;
    scale   : Interfaces.C.C_float) return Image;
  ...
  function GenImageCellular
   (width    : Interfaces.C.int;
    height   : Interfaces.C.int;
    tileSize : Interfaces.C.int) return Image;
  ...
  function GenImageText
   (width  : Interfaces.C.int;
    height : Interfaces.C.int;
    text   : Interfaces.C.Strings.chars_ptr) return Image;
  --  Generate image: grayscale image from text data
  pragma Import (C, GenImageText, "GenImageText");

  function GenImageText
   (width : Interfaces.C.int; height : Interfaces.C.int; text : String)
    return Image;
  --  Generate image: grayscale image from text data
  ...
  function ImageText
   (text     : Interfaces.C.Strings.chars_ptr;
    fontSize : Interfaces.C.int;
    color_p  : Color) return Image;
  --  Create an image from text (default font)
  pragma Import (C, ImageText, "ImageText");

  function ImageText
   (text : String; fontSize : Interfaces.C.int; color_p : Color) return Image;
  --  Create an image from text (default font)
  ...
  function ImageTextEx
   (font_p   : Font;
    ...
    spacing  : Interfaces.C.C_float;
    tint     : Color) return Image;
  ...
  procedure ImageAlphaCrop
   (image_p : access Image; threshold : Interfaces.C.C_float);
  --  Crop image depending on alpha value
  pragma Import (C, ImageAlphaCrop, "ImageAlphaCrop");

  procedure ImageAlphaClear
   (image_p : access Image; color_p : Color; threshold : Interfaces.C.C_float);
  ...
  procedure ImageBlurGaussian
   (image_p : access Image; blurSize : Interfaces.C.int);
  --  Apply Gaussian blur using a box blur approximation
  pragma Import (C, ImageBlurGaussian, "ImageBlurGaussian");

  procedure ImageResize
   (image_p   : access Image;
    newWidth  : Interfaces.C.int;
    newHeight : Interfaces.C.int);
  ...
  procedure ImageDither
   (image_p : access Image;
    rBpp    : Interfaces.C.int;
    gBpp    : Interfaces.C.int;
    bBpp    : Interfaces.C.int;
    aBpp    : Interfaces.C.int);
  ...
  function LoadImagePalette
   (image_p        : Image;
    maxPaletteSize : Interfaces.C.int;
    colorCount     : access Interfaces.C.int) return access Color;
  ...
  function GetImageAlphaBorder
   (image_p : Image; threshold : Interfaces.C.C_float) return Rectangle;
  --  Get image alpha border rectangle
  pragma Import (C, GetImageAlphaBorder, "GetImageAlphaBorder");

  function GetImageColor
   (image_p : Image; x : Interfaces.C.int; y : Interfaces.C.int) return Color;
  ...
  procedure ImageDrawPixelV
   (dst : access Image; position : Vector2; color_p : Color);
  ...
  procedure ImageDrawLineV
   (dst : access Image; start : Vector2; end_p : Vector2; color_p : Color);
  ...
  procedure ImageDrawRectangleV
   (dst : access Image; position : Vector2; size : Vector2; color_p : Color);
  --  Draw rectangle within an image (Vector version)
  pragma Import (C, ImageDrawRectangleV, "ImageDrawRectangleV");

  procedure ImageDrawRectangleRec
   (dst : access Image; rec : Rectangle; color_p : Color);
  --  Draw rectangle within an image
  ...
  function LoadTexture
   (fileName : Interfaces.C.Strings.chars_ptr) return Texture;
  ...
  function LoadTextureCubemap
   (image_p : Image; layout : CubemapLayout) return Texture;
  --  Load cubemap from image, multiple image cubemap layouts supported
  pragma Import (C, LoadTextureCubemap, "LoadTextureCubemap");

  function LoadRenderTexture
   (width : Interfaces.C.int; height : Interfaces.C.int) return RenderTexture;
  ...
  function IsRenderTextureReady
   (target : RenderTexture) return Interfaces.C.C_bool;
  ...
  procedure UpdateTextureRec
   (texture_p : Texture; rec : Rectangle; pixels : System.Address);
  ...
  procedure DrawTextureV
   (texture_p : Texture; position : Vector2; tint : Color);
  ...
  procedure DrawTextureRec
   (texture_p : Texture; source : Rectangle; position : Vector2; tint : Color);
  ...
  function ColorFromHSV
   (hue        : Interfaces.C.C_float;
    saturation : Interfaces.C.C_float;
    value      : Interfaces.C.C_float) return Color;
  ...
  function ColorBrightness
   (color_p : Color; factor : Interfaces.C.C_float) return Color;
  --  Get color with brightness correction, brightness factor goes from -1.0f to 1.0f
  pragma Import (C, ColorBrightness, "ColorBrightness");

  function ColorContrast
   (color_p : Color; contrast : Interfaces.C.C_float) return Color;
  --  Get color with contrast correction, contrast values between -1.0f and 1.0f
  pragma Import (C, ColorContrast, "ColorContrast");

  function ColorAlpha
   (color_p : Color; alpha : Interfaces.C.C_float) return Color;
  --  Get color with alpha applied, alpha goes from 0.0f to 1.0f
  pragma Import (C, ColorAlpha, "ColorAlpha");

  function ColorAlphaBlend
   (dst : Color; src : Color; tint : Color) return Color;
  ...
  function GetPixelColor
   (srcPtr : System.Address; format : PixelFormat) return Color;
  --  Get Color from a source pixel pointer of certain format
  pragma Import (C, GetPixelColor, "GetPixelColor");

  procedure SetPixelColor
   (dstPtr : System.Address; color_p : Color; format : PixelFormat);
  --  Set color formatted into destination pixel pointer
  pragma Import (C, SetPixelColor, "SetPixelColor");

  function GetPixelDataSize
   (width : Interfaces.C.int; height : Interfaces.C.int; format : PixelFormat)
    return Interfaces.C.int;
  ...
  function LoadFontEx
   (fileName       : Interfaces.C.Strings.chars_ptr;
    ...
    codepointCount : Interfaces.C.int) return Font;
  ...
  function LoadFontFromImage
   (image_p : Image; key : Color; firstChar : Interfaces.C.int) return Font;
  ...
  function LoadFontData
   (fileData       : System.Address;
    ...
    fontSize       : Interfaces.C.int;
    codepoints     : access Interfaces.C.int;
    codepointCount : Interfaces.C.int;
    type_p         : FontType) return access GlyphInfo;
  --  Load font data for further use
  pragma Import (C, LoadFontData, "LoadFontData");

  procedure UnloadFontData
   (glyphs : access GlyphInfo; glyphCount : Interfaces.C.int);
  ...
  function ExportFontAsCode
   (font_p : Font; fileName : Interfaces.C.Strings.chars_ptr)
    return Interfaces.C.C_bool;
  --  Export font as code file, returns true on success
  pragma Import (C, ExportFontAsCode, "ExportFontAsCode");

  function ExportFontAsCode
   (font_p : Font; fileName : String) return Interfaces.C.C_bool;
  --  Export font as code file, returns true on success
  ...
  procedure DrawTextCodepoints
   (font_p         : Font;
    codepoints     : access constant Interfaces.C.int;
    codepointCount : Interfaces.C.int;
    position       : Vector2;
    fontSize       : Interfaces.C.C_float;
    spacing        : Interfaces.C.C_float;
  ...
  function MeasureText
   (text : Interfaces.C.Strings.chars_ptr; fontSize : Interfaces.C.int)
    return Interfaces.C.int;
  --  Measure string width for default font
  pragma Import (C, MeasureText, "MeasureText");

  function MeasureText
   (text : String; fontSize : Interfaces.C.int) return Interfaces.C.int;
  --  Measure string width for default font
  ...
  function MeasureTextEx
   (font_p   : Font;
    text     : Interfaces.C.Strings.chars_ptr;
    fontSize : Interfaces.C.C_float;
    spacing  : Interfaces.C.C_float) return Vector2;
  ...
  function GetGlyphIndex
   (font_p : Font; codepoint : Interfaces.C.int) return Interfaces.C.int;
  --  Get glyph index position in font for a codepoint (unicode character), fallback to '?' if not found
  pragma Import (C, GetGlyphIndex, "GetGlyphIndex");

  function GetGlyphInfo
   (font_p : Font; codepoint : Interfaces.C.int) return GlyphInfo;
  --  Get glyph font info data for a codepoint (unicode character), fallback to '?' if not found
  pragma Import (C, GetGlyphInfo, "GetGlyphInfo");

  function GetGlyphAtlasRec
   (font_p : Font; codepoint : Interfaces.C.int) return Rectangle;
  --  Get glyph rectangle in font atlas for a codepoint (unicode character), fallback to '?' if not found
  pragma Import (C, GetGlyphAtlasRec, "GetGlyphAtlasRec");

  function LoadUTF8
   (codepoints : access constant Interfaces.C.int; length : Interfaces.C.int)
    return Interfaces.C.Strings.chars_ptr;
  ...
  function LoadCodepoints
   (text : Interfaces.C.Strings.chars_ptr; count : access Interfaces.C.int)
    return access Interfaces.C.int;
  --  Load all codepoints from a UTF-8 text string, codepoints count returned by parameter
  pragma Import (C, LoadCodepoints, "LoadCodepoints");

  function LoadCodepoints
   (text : String; count : access Interfaces.C.int)
    return access Interfaces.C.int;
  --  Load all codepoints from a UTF-8 text string, codepoints count returned by parameter
  ...
  function GetCodepoint
   (text          : Interfaces.C.Strings.chars_ptr;
    codepointSize : access Interfaces.C.int) return Interfaces.C.int;
  --  Get next codepoint in a UTF-8 encoded string, 0x3f('?') is returned on failure
  pragma Import (C, GetCodepoint, "GetCodepoint");

  function GetCodepoint
   (text : String; codepointSize : access Interfaces.C.int)
    return Interfaces.C.int;
  --  Get next codepoint in a UTF-8 encoded string, 0x3f('?') is returned on failure

  function GetCodepointNext
   (text          : Interfaces.C.Strings.chars_ptr;
    codepointSize : access Interfaces.C.int) return Interfaces.C.int;
  --  Get next codepoint in a UTF-8 encoded string, 0x3f('?') is returned on failure
  ...
  function CodepointToUTF8
   (codepoint : Interfaces.C.int; utf8Size : access Interfaces.C.int)
    return Interfaces.C.Strings.chars_ptr;
  --  Encode one codepoint into UTF-8 byte array (array length returned as parameter)
  pragma Import (C, CodepointToUTF8, "CodepointToUTF8");

  function TextCopy
   (dst : Interfaces.C.Strings.chars_ptr; src : Interfaces.C.Strings.chars_ptr)
    return Interfaces.C.int;
  ...
  function TextIsEqual
   (text1 : Interfaces.C.Strings.chars_ptr;
    text2 : Interfaces.C.Strings.chars_ptr) return Interfaces.C.C_bool;
  --  Check if two text string are equal
  pragma Import (C, TextIsEqual, "TextIsEqual");

  function TextIsEqual
   (text1 : String; text2 : String) return Interfaces.C.C_bool;
  --  Check if two text string are equal

  function TextLength
   (text : Interfaces.C.Strings.chars_ptr) return Interfaces.C.unsigned;
  ...
  function TextSubtext
   (text     : Interfaces.C.Strings.chars_ptr;
    ...
    length   : Interfaces.C.int) return Interfaces.C.Strings.chars_ptr;
  --  Get a piece of a text string
  pragma Import (C, TextSubtext, "TextSubtext");

  function TextSubtext
   (text : String; position : Interfaces.C.int; length : Interfaces.C.int)
    return String;
  --  Get a piece of a text string
  ...
  function TextInsert
   (text     : Interfaces.C.Strings.chars_ptr;
    ...
    position : Interfaces.C.int) return Interfaces.C.Strings.chars_ptr;
  --  Insert text in a position (WARNING: memory must be freed!)
  pragma Import (C, TextInsert, "TextInsert");

  function TextInsert
   (text : String; insert : String; position : Interfaces.C.int) return String;
  --  Insert text in a position (WARNING: memory must be freed!)

  function TextJoin
   (textList  : access Interfaces.C.Strings.chars_ptr;
    count     : Interfaces.C.int;
  ...
  procedure TextAppend
   (text : String; append : String; position : access Interfaces.C.int);
  --  Append text at specific position and move cursor!

  function TextFindIndex
   (text : Interfaces.C.Strings.chars_ptr;
    find : Interfaces.C.Strings.chars_ptr) return Interfaces.C.int;
  --  Find first text occurrence within a string
  pragma Import (C, TextFindIndex, "TextFindIndex");

  function TextFindIndex
   (text : String; find : String) return Interfaces.C.int;
  --  Find first text occurrence within a string
  ...
  procedure DrawCubeWiresV
   (position : Vector3; size : Vector3; color_p : Color);
  --  Draw cube wires (Vector version)
  pragma Import (C, DrawCubeWiresV, "DrawCubeWiresV");

  procedure DrawSphere
   (centerPos : Vector3; radius : Interfaces.C.C_float; color_p : Color);
  --  Draw sphere
  ...
  procedure DrawCylinder
   (position     : Vector3;
    radiusTop    : Interfaces.C.C_float;
    radiusBottom : Interfaces.C.C_float;
    height       : Interfaces.C.C_float;
    slices       : Interfaces.C.int;
    color_p      : Color);
  ...
  procedure DrawCylinderEx
   (startPos    : Vector3;
    endPos      : Vector3;
    startRadius : Interfaces.C.C_float;
    endRadius   : Interfaces.C.C_float;
    sides       : Interfaces.C.int;
    color_p     : Color);
  ...
  procedure DrawMesh
   (mesh_p : Mesh; material_p : Material; transform_p : Matrix);
  ...
  procedure DrawMeshInstanced
   (mesh_p     : Mesh;
    material_p : Material;
    transforms : access Matrix;
    instances  : Interfaces.C.int);
  --  Draw multiple mesh instances with material and different transforms
  pragma Import (C, DrawMeshInstanced, "DrawMeshInstanced");

  function ExportMesh
   (mesh_p : Mesh; fileName : Interfaces.C.Strings.chars_ptr)
    return Interfaces.C.C_bool;
  --  Export mesh data to file, returns true on success
  pragma Import (C, ExportMesh, "ExportMesh");

  function ExportMesh
   (mesh_p : Mesh; fileName : String) return Interfaces.C.C_bool;
  --  Export mesh data to file, returns true on success
  ...
  function GenMeshPoly
   (sides : Interfaces.C.int; radius : Interfaces.C.C_float) return Mesh;
  ...
  function GenMeshPlane
   (width  : Interfaces.C.C_float;
    length : Interfaces.C.C_float;
    resX   : Interfaces.C.int;
    resZ   : Interfaces.C.int) return Mesh;
  ...
  function GenMeshCube
   (width  : Interfaces.C.C_float;
    height : Interfaces.C.C_float;
    length : Interfaces.C.C_float) return Mesh;
  ...
  function GenMeshSphere
   (radius : Interfaces.C.C_float;
    rings  : Interfaces.C.int;
    slices : Interfaces.C.int) return Mesh;
  ...
  function GenMeshTorus
   (radius : Interfaces.C.C_float;
    size   : Interfaces.C.C_float;
    radSeg : Interfaces.C.int;
    sides  : Interfaces.C.int) return Mesh;
  --  Generate torus mesh
  ...
  function LoadMaterials
   (fileName      : Interfaces.C.Strings.chars_ptr;
    materialCount : access Interfaces.C.int) return access Material;
  --  Load materials from model file
  pragma Import (C, LoadMaterials, "LoadMaterials");

  function LoadMaterials
   (fileName : String; materialCount : access Interfaces.C.int)
  ...
  procedure UnloadModelAnimations
   (animations : access ModelAnimation_Array; animCount : Interfaces.C.int);
  --  Unload animation array data
  pragma Import (C, UnloadModelAnimations, "UnloadModelAnimations");

  function IsModelAnimationValid
   (model_p : Model; anim : ModelAnimation) return Interfaces.C.C_bool;
  ...
  function LoadWaveFromMemory
   (fileType : Interfaces.C.Strings.chars_ptr;
    fileData : System.Address;
    dataSize : Interfaces.C.int) return Wave;
  ...
  procedure UpdateSound
   (sound_p : Sound; data : System.Address; sampleCount : Interfaces.C.int);
  ...
  function ExportWave
   (wave_p : Wave; fileName : Interfaces.C.Strings.chars_ptr)
    return Interfaces.C.C_bool;
  --  Export wave data to file, returns true on success
  pragma Import (C, ExportWave, "ExportWave");

  function ExportWave
   (wave_p : Wave; fileName : String) return Interfaces.C.C_bool;
  --  Export wave data to file, returns true on success
  ...
  procedure WaveCrop
   (wave_p      : access Wave;
    initSample  : Interfaces.C.int;
    finalSample : Interfaces.C.int);
  --  Crop a wave to defined samples range
  pragma Import (C, WaveCrop, "WaveCrop");

  procedure WaveFormat
   (wave_p     : access Wave;
    sampleRate : Interfaces.C.int;
    sampleSize : Interfaces.C.int;
  ...
  function LoadMusicStream
   (fileName : Interfaces.C.Strings.chars_ptr) return Music;
  ...
  function LoadMusicStreamFromMemory
   (fileType : Interfaces.C.Strings.chars_ptr;
    data     : System.Address;
    dataSize : Interfaces.C.int) return Music;
  --  Load music stream from data
  pragma Import (C, LoadMusicStreamFromMemory, "LoadMusicStreamFromMemory");

  function LoadMusicStreamFromMemory
   (fileType : String; data : System.Address; dataSize : Interfaces.C.int)
    return Music;
  --  Load music stream from data
  ...
  function LoadAudioStream
   (sampleRate : Interfaces.C.unsigned;
    sampleSize : Interfaces.C.unsigned;
    channels   : Interfaces.C.unsigned) return AudioStream;
  --  Load audio stream (to stream raw audio pcm data)
  pragma Import (C, LoadAudioStream, "LoadAudioStream");

  function IsAudioStreamReady
   (stream : AudioStream) return Interfaces.C.C_bool;
  ...
  procedure SetAudioStreamVolume
   (stream : AudioStream; volume : Interfaces.C.C_float);
  --  Set volume for audio stream (1.0 is max level)
  pragma Import (C, SetAudioStreamVolume, "SetAudioStreamVolume");

  procedure SetAudioStreamPitch
   (stream : AudioStream; pitch : Interfaces.C.C_float);
  --  Set pitch for audio stream (1.0 is base level)
  pragma Import (C, SetAudioStreamPitch, "SetAudioStreamPitch");

  procedure SetAudioStreamPan
   (stream : AudioStream; pan : Interfaces.C.C_float);
  ...
  pragma
   Import
    (C, SetAudioStreamBufferSizeDefault, "SetAudioStreamBufferSizeDefault");

  procedure SetAudioStreamCallback
   (stream : AudioStream; callback : AudioCallback);
  --  Audio thread callback to request new data
  pragma Import (C, SetAudioStreamCallback, "SetAudioStreamCallback");

  procedure AttachAudioStreamProcessor
   (stream : AudioStream; processor : AudioCallback);
  --  Attach audio stream processor to stream, receives the samples as <float>s
  pragma Import (C, AttachAudioStreamProcessor, "AttachAudioStreamProcessor");

  procedure DetachAudioStreamProcessor
   (stream : AudioStream; processor : AudioCallback);
